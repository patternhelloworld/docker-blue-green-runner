...
sed -i -e 's/\r$//' .env .env.example.local .env.example.real .env.* *.yml  || echo "[NOTICE] Performed CRLF line ending inspection. There are no issues with the non-existent files."
